...
<template>
  <div class="chatbox" @input="userHasInteracted = true">
    <ChatHistoryBrowser
      ...
      v-show="showConversationBrowser"
    ...
    <div class="top-panel" :disabled="isSending" :style="{ zIndex: 10 }">
      ...
      <div class="top-panel-left" style="max-width: 150px">
        ...
        >
          {{ conversation_index + 1 }}/{{ conversation_histories.length }}
        </button>
        <button
          :title="
            conversation_index < conversation_histories.length - 1
              ? 'next chat'
              : 'new chat'
          "
          @click="next_chat"
    ...
    <Transition name="slide-up">
      <div
        class="input-area"
        @input="userHasInteracted = true"
        v-show="validID"
      >
        <textarea
          type="text"
          title="type a message to interact with the ai"
          ref="textarea"
          v-model="userMessage"
          ...
          @input="onUserTextInput"
        />
        <button
          title="send message"
          @click="sendMessage"
          :disabled="isSending || userMessage == ''"
          class="send-button"
        >
          <img
            class="brainIcon"
            src="../../assets/brain_glow.gif"
            v-show="!isSending"
            alt="brain"
          />
          <img
            class="brainIcon"
            src="../../assets/brain_quick.gif"
            v-show="isSending"
            alt="brain"
          />
        </button>
...
export default {
    ...
    return {
      ...
      parseableInserts: {
        roll: (args) => {
          ...
          const totalResult = individualRolls.reduce(
            (acc, curr) => acc + curr,
            0
          );
        ...
        roll4shoes: (args) => {
          ...
          const individualRolls = Array.from(
            { length: skillLevel },
            () =>
              // Add one roll to the initial skill level for the player
              Math.floor(Math.random() * 6) + 1
          );
          const opposedRolls = Array.from(
            { length: 6 },
            () =>
              // Generate 6 opposed rolls
              Math.floor(Math.random() * 6) + 1
          );
      ...
      guestPrompts: ['cogmate', 'raw', 'onramp', 'collab', 'grug'],

      kfcIDs: ['HCI530'],

      kfcPrompts: ['raw', 'KFCPT'],
        ...
        conversation_history: [
          {
            role: 'human',
            name: 'userID',
            content: "hey, you're a pretty chill bot.",
          },
          {
            role: 'ai',
            name: 'gpt-4',
          ...
          },
          {
            role: 'human',
            name: 'userID',
            content: "i'm kinda into this cognitive science thing.",
          },
          {
            role: 'ai',
            name: 'gpt-4',
            temp: this.temperature,
            prompt: this.prompts_id,
            content: 'oh, rad! i can totally help with that.',
          },
          {
            role: 'human',
            name: 'userID',
            content: "so what's this computational modeling biz?",
          },
          {
            role: 'ai',
            name: 'gpt-4',
            ...
            prompt: this.prompts_id,
            content:
              "it's like making a computer play pretend, but with cognitive processes.",
          },
          { role: 'human', name: 'userID', content: "and how's that work?" },
          {
            ...
            prompt: this.prompts_id,
            content:
              "you whip up a program that acts out the cognitive process you're curious about.",
          },
  ...
  computed: {
    message_list() {
      if (this.currentAIresponse.content != '') {
        return this.conversation_histories[this.conversation_index].concat({
          role: 'ai',
          content: this.currentAIresponse.content,
          name: this.model,
          temp: this.temperature,
          prompt: this.prompts_id,
        });
    ...
    nextDisabled() {
      ...
      let emptyList =
        this.conversation_histories[this.conversation_index].length == 0;
    ...
    validID() {
      return (
        this.studentIDs.includes(this.user_id) ||
        this.devIDs.includes(this.user_id) ||
        this.friendIDs.includes(this.user_id) ||
        this.guestIDs.includes(this.user_id) ||
        this.kfcIDs.includes(this.user_id)
      );
    },
    isDevID() {
  ...
  mounted() {
    ...
    this.conversation_histories = JSON.parse(
      localStorage.getItem('conversation_histories')
    ) || [[]];
    this.conversation_index =
      JSON.parse(localStorage.getItem('conversation_index')) || 0;
    ...
    this.showControlPanel =
      JSON.parse(localStorage.getItem('showControlPanel')) || false;
    //this.model = JSON.parse(localStorage.getItem('model')) || 'gpt-4-1106-preview';

    // sanity checks using min and max
    this.conversation_index = Math.max(
      0,
      Math.min(this.conversation_index, this.conversation_histories.length - 1)
    );
    ...
    this.musicality =
      JSON.parse(localStorage.getItem('musicality')) || 'Blues Scale';
  ...
  watch: {
    ...
    showControlPanel: function (newShowControlPanel) {
      localStorage.setItem(
        'showControlPanel',
        JSON.stringify(newShowControlPanel)
      );
    },
  },
  methods: {
    async onEnterKey(event) {
      if (/Mobi|Android/i.test(navigator.userAgent) || event.shiftKey == true) {
        ...
        this.userMessage =
          this.userMessage.slice(0, cursorPosition) +
          '\n' +
    ...
    async sendMessage() {
      ...
      if (this.userMessage.trim() !== '') {
        ...
        if (wasEmpty) {
          localStorage.setItem(
            'conversation_histories',
            JSON.stringify(this.conversation_histories)
          );
          localStorage.setItem(
            'conversation_index',
            JSON.stringify(this.conversation_index)
          );
        }
      }
    },
    parseInserts(inputMessage) {
      const pattern = /[?/](\w+)\((.*?)\)/g;
      const outputMessage = inputMessage.replace(
        pattern,
        (_, funcName, argsRaw) => {
    ...
    setupSocket() {
      ...
      if (!this.socket || this.socket.readyState == WebSocket.CLOSED) {
        this.socket = new WebSocket(
          'wss://' + this.backendURL + '/chat_stateless?token=' + this.api_token
        );
      }
    ...
    get_payload() {
      ...
      let payload = {
        ...
        conversation_history:
          this.conversation_histories[this.conversation_index],
    ...
    next_chat() {
      ...
      if (this.conversation_histories[this.conversation_index].length != 0) {
        if (this.conversation_index < this.conversation_histories.length - 1) {
          ...
          console.log(
            'moving to existing conversation: ' + this.conversation_index
          );
          localStorage.setItem(
            'conversation_index',
            JSON.stringify(this.conversation_index)
          ...
          this.conversation_index += 1;
          console.log(
            'creating new conversation at: ' + this.conversation_index
          );
        }
      }
    },
    ...
    downloadFile() {
      ...
      let formattedConversation = this.conversation_histories[
        this.conversation_index
      ]
        .map((message) => {
      ...
      let blob = new Blob([formattedConversation], {
      ...
      this.saveToServer(
        ...
        filename,
        extension,
        this.use_case.concat([this.user_id])
      );
    ...
    scrollCheck() {
      ...
      if (chatHistory) {
        ...
        if (
          chatHistory.scrollTop + chatHistory.clientHeight + threshold >=
    ...
    setupMusicality() {
      ...
      this.scaleNotes = this.generateScale(
        this.notesInScale,
        this.baseNote,
        this.scalePatterns[this.musicality]
      );
    ...
    async playTickSound() {
      ...
      this.currentNotePointer =
        (this.currentNotePointer + this.scaleNotes.length) %
        this.scaleNotes.length;
...
.top-panel input,
.top-panel select,
.top-slide-panel input,
.top-slide-panel select,
.top-panel-left input,
.top-panel-left seleect {
  background-color: var(--base-color);
  border-width: 2px;
  border-color: #6d2fff;
  border-radius: 10px;
...
.top-panel button,
.top-panel span,
.top-slide-panel button,
.top-slide-panel span {
...
.top-slide-panel button:active,
.send-button:active {
  background: var(--button-focus-color);
}

.top-panel button:disabled,
.top-slide-panel button:disabled {
  opacity: 0.3;
}
